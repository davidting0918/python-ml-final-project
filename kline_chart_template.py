...
def update_time_unit_callback(attr, old, new):
    global price_data, p, source, raw_price_data
    new_renderers_list = []
    ...
    p.segment(source=source, x0='open_time', x1='open_time', y0='H', y1='L', color="black", name="segment")
    p.vbar(source=source, view=inc_cds, x='open_time', width=timeunit_width_lookup_table[time_select.value], top='O', bottom='C', fill_color="#D5E1DD",
           line_color="green", name="inc")
    p.vbar(source=source, view=dec_cds, x='open_time', width=timeunit_width_lookup_table[time_select.value], top='O', bottom='C', fill_color="#F2583E",
           line_color="red", name='dec')
    ...
    p.x_range.start = new_x_range_start
    p.x_range.end = new_x_range_end
    p.x_range.reset_start = new_x_range_start
    p.x_range.reset_end = new_x_range_end
